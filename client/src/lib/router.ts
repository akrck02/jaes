...
export type routeMap = { [key: string]: viewHandler };
...
export function setRoutes(routes: routeMap) {
  for (const route in routes) {
    setRoute(route, routes[route]);
  }
}
...
export function showRoute(path: string, container: HTMLElement) {
  ...
  for (const route of keys) {
    ...
    if (paths && null != params && 0 != params.length) {
      const routeCallback = paths.get(route);
      if (routeCallback) {
        routeCallback(params.slice(1), container);
      }
      return;
    }
  }
